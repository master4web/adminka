function createjsDOMenu() {
  ...
  with (absoluteMenu4) {
    ...
    addMenuItem(new menuItem("-"));
	addMenuItem(new menuItem("Комментарии к статьм", "", script + '?admin=comments_publs'));
	addMenuItem(new menuItem("Комментарии к новостям", "", script + '?admin=comments_news'));
	addMenuItem(new menuItem("-"));
...
absoluteMenu7 = new jsDOMenu(140, "absolute");
 ...
 absoluteMenu9 = new jsDOMenu(180, "absolute");
  with (absoluteMenu9) {
    addMenuItem(new menuItem("Сотрудников", "", script+'?admin=blog'));
	}
